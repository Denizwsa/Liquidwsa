...
import net.ccbluex.liquidbounce.integration.theme.component.components.native.NativeArrayListComponent
import net.ccbluex.liquidbounce.integration.theme.component.components.native.NativeEffectsComponent
import net.ccbluex.liquidbounce.integration.theme.component.components.native.NativeInventoryComponent
import net.ccbluex.liquidbounce.integration.theme.component.components.native.NativeTargetHudComponent
...
object ModuleHud : ClientModule("HUD", ModuleCategories.RENDER, state = true, hide = true) {
    ...
    val components = tree(ValueGroup("AdditionalComponents")).apply {
        ...
        tree(NativeArrayListComponent)
        tree(NativeEffectsComponent)
        tree(NativeTargetHudComponent)
        tree(NativeInventoryComponent)
    }
    ...
    override fun onEnabled() {
        ...
        // Native HUD theme renders without the CEF overlay (see ThemeManager.isNativeActive)
        if (isVisible && !ThemeManager.isNativeActive) {
            overlay.open()
        }
    }
    ...
    private val screenHandler = handler<ScreenEvent> { event ->
        // Close the tab when the HUD is not running, is hiding now, the player is not in-game,
        // or the native HUD theme is active (no CEF overlay in that mode).
        if (!enabled || !isVisible || ThemeManager.isNativeActive) {
            overlay.close()
            return@handler
        }
