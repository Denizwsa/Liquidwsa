...
object CommandClientThemeSubcommand {
    ...
    private fun setSubcommand() = CommandBuilder.begin("set")
        ...
        .suspendHandler {
            ...
            // Native (non-browser) HUD renderer. Not a real Theme - stored as a sentinel.
            if (idOrUrl.equals(ThemeManager.NATIVE_THEME_ID, true)) {
                runCatching {
                    ThemeManager.selectTheme(ThemeManager.NATIVE_THEME_ID)
                    ConfigSystem.store(ThemeManager)
                }.onFailure {
                    chat(markAsError("Failed to switch theme: ${it.message}"))
                }.onSuccess {
                    chat(regular("Switched theme to "), variable("Native").copyable(), regular("."))
                }
