...
object ThemeManager : Config("theme") {
    ...
    /**
     * Pseudo theme id used to switch to the in-built native (non-browser) HUD renderer.
     * It is not a real [Theme] (it has no web assets/URL); it is stored as a sentinel in [currentTheme].
     */
    const val NATIVE_THEME_ID = "native"

    internal val themesFolder = File(ConfigSystem.rootFolder, "themes")

    val themes: List<Theme>
        field = mutableListOf()
    val themeIds get() = themes.map { theme -> theme.metadata.id } + NATIVE_THEME_ID

    /**
     * Whether the native HUD theme is currently selected. When true, the CEF HUD overlay is kept
     * closed (see [ModuleHud]) and native HUD components render themselves instead.
     */
    val isNativeActive: Boolean
        get() = currentTheme.equals(NATIVE_THEME_ID, true)
    ...
    /**
     * Selects a theme by its id. Accepts the [NATIVE_THEME_ID] sentinel as well as any loaded theme id.
     * Setting [currentTheme] triggers the change side effects (ScreenManager/ModuleHud/ModuleClickGui).
     */
